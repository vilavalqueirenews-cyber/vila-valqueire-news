/*
 * ============================================
 * VILA VALQUEIRE NEWS - Netlify Function
 * Busca noticias REAIS e atualizadas dos feeds
 * RSS de fontes confiaveis (G1, UOL) no servidor.
 *
 * Sem dependencias externas (usa fetch nativo do
 * Node 18+ disponivel no Netlify). Roda no servidor,
 * sem problemas de CORS e sem depender de servicos
 * de terceiros (ex: rss2json).
 * ============================================
 */
...
exports.handler = async (event) => {
    // Configuracoes CORS (permitir qualquer origem)
    const headers = {
    ...
    // Parametro opcional de categoria: /.netlify/functions/news?cat=politica
    let categoryFilter = null;
    if (event.queryStringParameters && event.queryStringParameters.cat) {
        const raw = event.queryStringParameters.cat.toLowerCase();
        if (Object.keys(CATEGORY_KEYWORDS).indexOf(raw) !== -1) {
            categoryFilter = raw;
        }
    }
    ...
    try {
        ...
        const fetchPromises = SOURCES.map(async (source, index) => {
            try {
                ...
                const res = await fetch(source.url, {
                    ...
                    headers: { 'User-Agent': 'Mozilla/5.0 (VilaValqueireNews bot; +https://www.vilavalqueirenews.com.br)' }
                });
        ...
        return {
            statusCode: 200,
            headers,
            body: JSON.stringify({ status: 'ok', count: allNews.length, items: allNews })
        };
    } catch (err) {
        ...
        return {
            statusCode: 500,
            headers,
            body: JSON.stringify({ status: 'error', message: err.message, items: [] })
        };
    }
};
...
function parseRss(xml) {
    ...
    function extractDescription(block) {
        // description pode conter html; extrair texto limpo
        let d = extractTag(block, 'description');
        return d;
    }
...
// (preserva a URL inteira sem quebrar)
function cleanUrl(u) {
    return String(u || '')
        .replace(/<!\[CDATA\[/g, '')
        .replace(/\]\]>/g, '')
...
// Se remover as tags primeiro (regex <[^>]*>), o <![CDATA[...]]> e 
// destruido junto com o texto (o '<' do CDATA engole ate o primeiro '>').
